...
class PurchaseReturnIntegrationTest {
  ...
  void createPurchaseReturn_ShouldSucceed() throws Exception {
    PurchaseReturnLineDto lineDto = PurchaseReturnLineDto.builder()
    ...
    PurchaseReturnDto returnDto = PurchaseReturnDto.builder()
    ...
    Assertions.assertEquals(0, stockLedgerRepository.findAll().get(0).getQtyOut().compareTo(new BigDecimal("5")));

    // Verify GL Transaction
    Assertions.assertEquals(1, glTransactionRepository.count());
    Assertions.assertEquals(4, glLineRepository.count(), "Should create 4 GL lines (AP, Purchase Return, CGST, SGST)");
  }
}
