  ...
  useEffect(() => {
    async function loadMasters() {
      ...
      try {
        ...
        const defaultCustomerId = p.find((x) => x.type === "CUSTOMER")?.id ?? ""
        const defaultWarehouseId = w[0]?.id ?? ""
        const defaultItem = i[0]

        setPartyId(defaultCustomerId)
   ...
   async function onCreateDemo() {
     ...
     try {
      if (!partyId || !warehouseId || !itemId) {
        throw new Error("Select customer, warehouse, and item")
      }
   ...
   return (
     <div className="space-y-6">
       <PageHeader
         ...
         description="Create a sales invoice. Coming Soon: full line editor."
         actions={
       ...
       <div className="rounded-2xl border border-border p-4">
        ...
        ) : customers.length === 0 || warehouses.length === 0 || items.length === 0 ? (
          <div className="mt-3 text-sm text-muted-foreground">
            Missing masters. Create at least one customer, warehouse, and item in Masters first.
          </div>
        ) : (
